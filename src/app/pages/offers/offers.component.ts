...
@Component({
  selector: 'app-offers',
  imports: [BannerSliderComponent,   CommonModule,
    FormsModule,
    DropdownModule,
    CheckboxModule,
    SliderModule,
 ...
 currentLang = 'en';
    ...
    this.productsService.getAllProducts().subscribe({
      next: (res) => {
        ...
        const offers = products.filter((p: any) => 
          p.priceAfterDiscount !== 0 && p.price !== p.priceAfterDiscount
      );
      this.productsList.set(offers)
  ...
  getBrands() {
    this.brandsService.getAllBrands().subscribe({
      next: (res) => {
        this.brandsList = res.brands;  // Use the brands directly without transformation
      },
      error: (err) => console.log(err)
    });
  }
  ...
  private mapQueryParamsToForm(params: any): any {
    const keyMap: { [key: string]: string } = {
      ...
      brand: 'brand', // ✅ map brand from URL to form
      country: 'country',
      draw: 'tireDrawType',
  ...
  // Add this method to automatically apply filters when any dropdown changes
  onFilterChange() {
    this.applyFilters();
  }
